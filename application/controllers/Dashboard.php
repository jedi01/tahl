...
class Dashboard extends BRT_Controller {
  ...
  $this->navMeta = ["active" => "clientList", "pageTitle" => "Client List", "bc" => array(
       ...
       /* $this->navMeta = ["active" => "clientList", "pageTitle" => "Client List", "bc" => array(
        ...
        $this->db->where('traderID', $this->getUserID());
        $this->db->select('*');
        $this->db->from('clientList');
        $query = $this->db->get();
        $result['userList'] = $query->result_array(); */
        $this->view("dashboard/clientList");
    }
...
//        $this->form_validation->set_rules("allocationToFR", "AllocationTo FR Check box Can not be blank", "required|trim");
           ...
           // if($this->input->post('SoftDollars') > 0){
            ...
            for($i=0;$i<$total_rows;$i++){
                if($trade_val[$i]['Shares'] > 0){
                    ...
                    if(isset($trade_val[$i]['Side'])){
                        $side = $trade_val[$i]['Side'];
                    }
                    if(isset($trade_val[$i]['Symbol'])){
                        $Symbol = $trade_val[$i]['Symbol'];
                    }
                    if(isset($trade_val[$i]['AveragePrice'])){
                        $AveragePrice = number_format((float)$trade_val[$i]['AveragePrice'], 2, '.', '');
                    }

                    $saveData['Side'] = $side;
                    $saveData['Shares'] = $trade_val[$i]['Shares'];
                    $saveData['Symbol'] = $Symbol;
                    $saveData['AveragePrice'] = $AveragePrice;
            ...
            if(!empty($saveData['SoftDollars']) && $saveData['SoftDollars'] != 0) {
            ...
            if($clist_commission > $saveData['Commission'] || $clist_commission < $saveData['Commission']){
    ...
    function AccountCoverageList(){
        ...
        $this->meta["pageCss"] = array(
         ...
         $this->data["accountCoverages"] = $this->mdb->getData(TAB_Coverage);
