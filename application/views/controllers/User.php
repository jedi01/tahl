...
class User extends BRT_Controller {
    ...
    function profile() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->setAlertMsg("Profile Changed!", SUCCESS);
        } else {
    ...
    function addUser() {
        ...
        $this->form_validation->set_rules("firstName", "FirstName Can not be blank", "required|trim");
        $this->form_validation->set_rules("email", "Email Can not be blank", "required|trim|is_unique[users.email]");
        $this->form_validation->set_rules("password", "Password Can not be blank", "required|trim");
        $this->form_validation->set_rules("lastName", "LastName Can not be blank", "required|trim");

        if ($this->form_validation->run() == true) {
            $cols = ["firstName", "lastName", "email", "phoneNumber", "position"];
            foreach ($cols as $col) {
                $saveData[$col] = $this->input->post($col);
            }
            // $saveData["birthDate"] = changeDateFormat($this->input->post("birthDate"));
    ...
    function editUser($id) {
        ...
        if ($this->form_validation->run() == true) {
            $cols = ["position", "email", "phoneNumber", "firstName", "lastName"];
            foreach ($cols as $col) {
                $saveData[$col] = $this->input->post($col);
            }
    ...
    function emailUserCheck(){
        $position = $_REQUEST['position'];
        $flag = 1;
        $users = $this->mdb->countData(TAB_USERS, ["email" => $_REQUEST["email_id"]]);
        if ($users) {
            $flag = 0;
        }
        echo json_encode(
                [
                    "value" => $_REQUEST["email_id"],
                    "valid" => $flag,
                    "message" => "Duplicate Email!"
                ]
        );
