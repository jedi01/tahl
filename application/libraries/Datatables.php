 ...
 * Created on : May 13, 2018, 8:53:29 AM
 */
class Datatables {
    ...
    public function select($columns) {
        ...
        for ($i = 0; $i < intval($_POST["iColumns"]); $i++) {
            if (in_array($_POST["mDataProp_$i"], $columns)) {
              //  $this->extra["io"][$i] = $_POST["mDataProp_$i"];
                array_push($mDataProp, $_POST["mDataProp_$i"]);
            }
        }
       // $this->extra["hello"] = $columns;
        ...
        $newWhere = '';
        if ($_POST["sSearch"]) {
            foreach ($mDataProp as $key=>$col) {
				
						$whereOr[$col] = $_POST["sSearch"];
						if($key == 0){
							$newWhere .= "(".$col." LIKE '%".$_POST["sSearch"]."%' ";
						}else{
							$newWhere .= "OR ".$col." LIKE '%".$_POST["sSearch"]."%' ";
						}
