...
class Mdb extends CI_Model {
    ...
    function getTrade($where,$startDate,$endDate) { 
       
        $this->db->select('*');
        $this->db->from(TAB_Trade);
        $this->db->join("clientList","clientList.clientID = Trade.Client");
        $this->db->join("users","users.id = clientList.traderID");
        $this->db->where($where);
        $this->db->where('Trade.Date >=', $startDate);
        $this->db->where('Trade.Date <=', $endDate);
        $query = $this->db->get();
        if ($query->num_rows()) {
            return $query->result();
        }
    ...
    function getSingleRow($table, $where = 0, $order = 0, $select = 0) {
        if ($select) {
            $this->db->select($select);
        } else {
            $this->db->select('*');
    ...
    function removeData($table, $where) {
     ...
     function getclientdata($clientid) {
     	...
     	$sql="SELECT * FROM `clientList` WHERE `clientID`='".$clientid."'";
     	$quy = $this->db->query($sql);
     	return $quy->result_array();
       
    }
    
     function gettraderdata($traderID) {
     
     	$sql="SELECT `firstName`,`lastName` FROM `users` WHERE `id`='".$traderID."'";
     	$quy = $this->db->query($sql);
     	return $quy->result_array();
       
    }
    
    function getclientcommission($clist_name,$clist_account) {
     
     	 $sql="SELECT `clientName`,`accountName`,`commission` FROM `clientList` WHERE `clientName`='".$clist_name."' AND `accountName`='".$clist_account."'";
     	$quy = $this->db->query($sql);
     	return $quy->result_array();
    ...
    public function tmtsub()
    {
        $this->db->select("*");
        $this->db->from(TAB_TmtSubscription);
        $this->db->join("account_coverage","account_coverage.id = tmt_subscription.accountName");
